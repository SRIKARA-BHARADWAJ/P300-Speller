...
import os
#%% Processing of Subject A

# Load Subject A Raw Data
Subject_A_Train_Raw = sio.loadmat(r"S:\BE\Mini Project\Dataset\Subject_A_Train.mat")
...
# Filter the Data Using Butterworth bandpass filter from 0.1Hz to 20Hz & order 4
...
# 85 - characters, 4320 - # of samples where intensification is 1, 64 - channels
# 160 time window - each sample is taken after 160ms
...
# 12 - Store the signal samples in 12 buffers
# 12 bcz there are totally 12 rows and columns in the matrix
# 24 bcz # of 1's repeating in dataset is for 24 samples
...
# 288 bcz each row/column is intensified 15 times per character, so 4320/15 = 288
# so, we need only 288 samples for each character to get p300 wave
...
# Properly dividing values of dataset so that train and test data have varieties of samples
print('Shuffling...')
...
# a_code_train = first 80% of samples, a_code_test = last 20% of samples


# format data as input tensor shape
# Done according to the documentation of tensorflow library
...
# Save data
# Save the train and test data in specific folder
base_dir = r"S:\BE\Mini Project\Codes\Train and Test Data"
os.chdir(base_dir)
print('Saving data for Subject A...')
np.save('a_signal_train.npy',x_train)
np.save('a_signal_test.npy',x_test)
np.save('a_type_train.npy',y_train)
np.save('a_type_test.npy',y_test)
np.save('a_code_train.npy',a_code_train)
np.save('a_code_test.npy',a_code_test)
...
# Change directory for saving test characters
base_dir = r"S:\BE\Mini Project\Codes\Test Characters For GUI"
os.chdir(base_dir)
...
for c in range(1,18):
    ...
    np.save(file_name, joint)

#%% Please delete all the variables from previous cell run before running the next cell to avoid MemoryError




#%% Processing of Subject B
 
# Load Subject B Raw Data
print('\n\nNow processing Subject B data...\n\n')
Subject_B_Train_Raw = sio.loadmat(r"S:\BE\Mini Project\Dataset\Subject_B_Train.mat")
...
np.save('b_signal_train.npy',x_train)
np.save('b_signal_test.npy',x_test)
np.save('b_type_train.npy',y_train)
np.save('b_type_test.npy',y_test)
np.save('b_code_train.npy',b_code_train)
np.save('b_code_test.npy',b_code_test)
...
#%% Clear all variables and proceed further to avoid MemoryError


#%% Subject A+B processing
 
# Combine Subject's A and B data
b_train =np.load(r'S:\BE\Mini Project\Codes\Train and Test Data\b_signal_train.npy')
b_type = np.load(r'S:\BE\Mini Project\Codes\Train and Test Data\b_type_train.npy')
a_train =np.load(r'S:\BE\Mini Project\Codes\Train and Test Data\a_signal_train.npy')
a_type = np.load(r'S:\BE\Mini Project\Codes\Train and Test Data\a_type_train.npy')
...
np.save('ab_signal_train.npy',ab_signal)
np.save('ab_type_train.npy',ab_type)
